  ...
  try {
    switch (frame.type) {
      ...
      case "chat.send": {
        ...
        // Runs alongside the turn, never before it — titling a session must
        // not be the reason a message is slow to send.
        autoTitleIfUntitled(gateway, sessionKey, frame.text);
