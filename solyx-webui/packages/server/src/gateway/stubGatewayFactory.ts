...
function simulateReply(sessionKey: string, userText: string, emit: (event: GatewayEvent) => void): void {
  // Deliberately markdown, and deliberately varied: the real agent answers in
  // markdown, so a plain-prose fixture meant local dev could never show
  // whether the chat pane actually renders it. It streams word by word like
  // any other reply, which also exercises the half-finished syntax a reader
  // sees mid-stream.
  const reply = [
    `## Draft bijgewerkt`,
    ``,
    `Received "${userText}" and updated the draft page. Changes:`,
    ``,
    `- **Kop** herschreven`,
    `- Prijstabel toegevoegd`,
    ``,
    `| Product | Prijs |`,
    `| --- | --- |`,
    `| Nymo | 1.995 |`,
    `| Nymo met boiler | 2.795 |`,
    ``,
    "```html",
    `<p>Voorbeeldblok</p>`,
    "```",
    ``,
    `Bekijk de [draftpagina](https://example.com/draft) voordat je publiceert.`,
  ].join("\n");
  let cumulative = "";
